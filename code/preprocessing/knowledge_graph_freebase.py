...
class KonwledgeGraphFreebase:
    ...
    def search_one_hop(self, src:str, rel:str) -> List[Tuple[str]]:
        topic_entity = src
    ...
    def search_two_hop(self, src:str, rels:Tuple[str,str])->List[Tuple[str,str]]:
        topic_entity = src
    ...
    def deduce_leaves_by_path(self, src: str, path: List[str], no_hop_flag: str = 'END OF HOP') -> Tuple[str]:
        ...
        assert len(path) <= 2
        if len(path) == 1:
            return self.search_one_hop(src, path[0])
        elif len(path) == 2:
            return self.search_two_hop(src, path)
        assert ValueError
