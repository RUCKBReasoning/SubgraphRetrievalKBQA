    ...
    for epoch in range(cfg['num_epoch']):
        try:
            ...
            for iteration in tqdm(range(train_data.num_data // cfg['batch_size'])):
                ...
                pred = pred.cpu().numpy()
                acc, max_acc, _ = cal_accuracy(pred, batch[-1])
...
def inference(my_model, valid_data, entity2id, cfg, log_info=True):
    # Evaluation
    my_model.eval()
    ...
    for iteration in tqdm(range(valid_data.num_data // test_batch_size)):
        ...
        # pred = pred.data.cpu().numpy()
        pred = pred.cpu().numpy()
        acc, max_acc, hits = cal_accuracy(pred, batch[-1])
        if log_info:
            output_pred_dist(pred_dist, batch[-1], id2entity, iteration * test_batch_size, valid_data, hits, f_pred)
