...
def get_config(config_path="config.yml"):
    with open(config_path, "r") as setting:
        config = yaml.load(setting, yaml.SafeLoader)
    return config
    ...
    for next_id, document in tqdm(documents.items()):
        ...
        for entity in document['document']['entities']:
            ...
            if entity['start'] + title_len + 1 >= max_document_word:
                continue
            entity_len = min(
                max_document_word, entity['end'] + title_len + 1) - (entity['start'] + title_len + 1)
            global_entity_ids += [entity2id[entity['text']]] * entity_len
            word_ids += range(entity['start'] + title_len + 1,
                              entity['start'] + title_len + 1 + entity_len)
            if entity_len != 0:
                word_weights += [1.0 / entity_len] * entity_len

        assert len(word_weights) == len(word_ids)
        document_entity_indices[next_id] = (
            global_entity_ids, word_ids, word_weights)
...
def cal_accuracy(pred, answer_dist):
    ...
    for i, l in enumerate(pred):
        ...
        hits.append(1.0 if answer_dist[i, l] != 0 else 0.0)
    for dist in answer_dist:
        if np.sum(dist) != 0:
            num_answerable += 1
    return num_correct / len(pred), num_answerable / len(pred), hits

def output_pred_dist(pred_dist, answer_dist, id2entity, start_id, data_loader, hits, f_pred):
    for i, p_dist in enumerate(pred_dist):
        data_id = start_id + i
        l2g = {l: g for g,
               l in data_loader.global2local_entity_maps[data_id].items()}
        output_dist = {id2entity[l2g[j]]: float(prob) for j, prob in enumerate(
            p_dist.data.cpu().numpy()) if j < len(l2g)}
        answers = [answer['text'] if type(answer['kb_id']) == int else answer['kb_id']
                   for answer in data_loader.data[data_id]['answers']]
        # f_pred.write(json.dumps({'answers': answers, 'ent_size': len(output_dist),
        #              'seeds': data_loader.data[data_id]['entities'], 'hit': hits[i]}) + '\n')

        f_pred.write(json.dumps({'dist': output_dist, 'answers': answers,
                     'seeds': data_loader.data[data_id]['entities'], 'tuples': data_loader.data[data_id]['subgraph']['tuples']}) + '\n')
...
def sparse_bmm(X, Y):
    ...
    lookup = Y[I[0, :], I[2, :], :]
    X_I = torch.stack(
        (I[0, :] * M + I[1, :], use_cuda(torch.arange(Z).type(torch.LongTensor))), 0)
    S = use_cuda(Variable(torch.cuda.sparse.FloatTensor(
        X_I, V, torch.Size([B * M, Z])), requires_grad=False))
    prod_op = LeftMMFixed()
...
def read_padded(my_lstm, document_emb, document_mask):
    ...
    document_lengths = torch.sum(document_mask, dim=1).type(
        'torch.IntTensor')  # num_document
    ...
    # skip warning: length could be 0
    document_lengths_np[document_lengths_np == 0] = 1
    ...
    document_emb = pack_padded_sequence(
        document_emb, document_lengths_np, batch_first=True)  # padded array
    # [batch_size * max_relevant_doc, max_document_word, entity_dim * 2] [2, batch_size * max_relevant_doc, entity_dim]
    document_emb, hidden = my_lstm(document_emb, hidden)
    document_emb, _ = pad_packed_sequence(document_emb, batch_first=True)

    document_emb = document_emb[use_cuda(inverse_perm_idx)]
    hidden = (hidden[0][:, use_cuda(inverse_perm_idx), :],
              hidden[1][:, use_cuda(inverse_perm_idx), :])
    batch_max_document_word = document_emb.size()[1]
    if batch_max_document_word < max_document_word:
        all_zeros = use_cuda(Variable(torch.zeros(
            (num_document, max_document_word, hidden_size * num_layer))))
        all_zeros[:, : batch_max_document_word, :] = document_emb
        document_emb = all_zeros

    assert (num_document, max_document_word,
            hidden_size * num_layer) == document_emb.size()
