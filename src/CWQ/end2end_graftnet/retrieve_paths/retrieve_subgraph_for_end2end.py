...
print(f'retrieval_model_ckpt {retrieval_model_ckpt}')
print(f'TOP_K {TOP_K}, MAX_HOP_LENGTH {MAX_HOP_LENGTH}')
...
def entity_conversion(ent):
    ent = {"kb_id":"<fb:"+str(ent)+">","text":"<fb:"+str(ent)+">"}
    return ent

def relation_conversion(rel):
    rel = {"rel_id":"<fb:"+rel+">","text":"<fb:"+rel+">"}
    return rel
...
def retrieve_subgraph(json_obj: Dict[str, Any], entities):
    ...
    for entity_id in entities_cid:
        ...
        path_score_list = infer_paths_from_kb(
            question, topic_entity, TOP_K, TOP_K, MAX_HOP_LENGTH)
        nodes = []
        ...
        for path, score in path_score_list:
            ...
            partial_nodes = [e for e in partial_nodes]
            ...
            data_list.append({
                "id": json_obj["id"],
                "question": json_obj["question"],
                "entities": json_obj["entities"],#[ent for ent in json_obj["entities_cid"]],
                "entities_cid": json_obj["entities_cid"],
                "answers": json_obj["answers"],#[ent for ent in json_obj['answers_cid']],
                "answers_cid": json_obj["answers_cid"],
...
def run():
    ...
    dump_data_folder = "../tmp/reader_data/CWQ_end2end/"
    ...
    for json_obj in tqdm(train_dataset, desc="retrieve:train"):
        train_new.extend(retrieve_subgraph(json_obj, entities))


    dump_jsonl(train_new, os.path.join(
