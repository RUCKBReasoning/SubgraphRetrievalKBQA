...
class DataLoader():
    def __init__(self, data_file, documents, document_entity_indices, document_texts, word2id, relation2id, entity2id, max_query_word, max_document_word, use_kb, use_doc, use_inverse_relation):
        ...
        with open(data_file) as f_in:
            for line in tqdm(f_in):
                ...
                self.max_relevant_doc = max(
                    self.max_relevant_doc, len(line['passages']))
                self.max_facts = max(self.max_facts, 2 *
                                     len(line['subgraph']['tuples']))
        ...
        self.q2e_adj_mats = np.zeros(
            (self.num_data, self.max_local_entity, 1), dtype=float)
        self.query_texts = np.full(
            (self.num_data, self.max_query_word), len(self.word2id), dtype=int)
        self.rel_document_ids = np.full(
            (self.num_data, self.max_relevant_doc), -1, dtype=int)  # the last document is empty
    ...
    def _prepare_data(self):
        ...
        for sample in tqdm(self.data):
            ...
            if self.use_kb:
                for i, tpl in enumerate(sample['subgraph']['tuples']):
                    ...
                    if not self.use_inverse_relation:
                        ...
                        self.kb_fact_rels[next_id,
                                          i] = self.relation2id[rel['text']]
                    else:
                        entity2fact_e += [g2l[self.entity2id[sbj['text']]],
                                          g2l[self.entity2id[obj['text']]]]
                        entity2fact_f += [2 * i, 2 * i + 1]
                        fact2entity_f += [2 * i, 2 * i + 1]
                        fact2entity_e += [g2l[self.entity2id[obj['text']]],
                                          g2l[self.entity2id[sbj['text']]]]
                        self.kb_fact_rels[next_id, 2 *
                                          i] = self.relation2id[rel['text']]
                        self.kb_fact_rels[next_id, 2 * i +
                                          1] = self.relation2id[rel['text']] + len(self.relation2id)
            ...
            for j, entity in enumerate(sample['entities']):
                ...
                self.q2e_adj_mats[next_id,
                                  g2l[self.entity2id[entity['text']]], 0] = 1.0
            ...
            if self.use_doc:
                for j, passage in enumerate(sample['passages']):
                    ...
                    if document_id not in self.document_entity_indices:
                        continue
                    (global_entity_ids, word_ids,
                     word_weights) = self.document_entity_indices[document_id]
                    entity_pos_local_entity_id += [g2l[global_entity_id]
                                                   for global_entity_id in global_entity_ids]
                    entity_pos_word_id += [word_id + j *
                                           self.max_document_word for word_id in word_ids]
            ...
            for answer in sample['answers']:
                ...
                if self.entity2id[answer[keyword]] in g2l:
                    self.answer_dists[next_id,
                                      g2l[self.entity2id[answer[keyword]]]] = 1.0

            self.kb_adj_mats[next_id] = (np.array(entity2fact_f, dtype=int), np.array(entity2fact_e, dtype=int), np.array(
                [1.0] * len(entity2fact_f))), (np.array(fact2entity_e, dtype=int), np.array(fact2entity_f, dtype=int), np.array([1.0] * len(fact2entity_e)))
            self.entity_poses[next_id] = (
                entity_pos_local_entity_id, entity_pos_word_id, entity_pos_word_weights)
    ...
    def _build_kb_adj_mat(self, sample_ids, fact_dropout):
        ...
        for i, sample_id in enumerate(sample_ids):
            (mat0_0, mat0_1, val0), (mat1_0, mat1_1,
                                     val1) = self.kb_adj_mats[sample_id]
            assert len(val0) == len(val1)
            num_fact = len(val0)
            num_keep_fact = int(np.floor(num_fact * (1 - fact_dropout)))
            mask_index = np.random.permutation(num_fact)[: num_keep_fact]
            # mat0
            mats0_batch = np.append(mats0_batch, np.full(
                len(mask_index), i, dtype=int))
            ...
            # mat1
            mats1_batch = np.append(mats1_batch, np.full(
                len(mask_index), i, dtype=int))
    ...
    def get_batch(self, iteration, batch_size, fact_dropout):
        ...
        *** return values ***
        ...
        """
        sample_ids = self.batches[batch_size *
    ...
    def _build_document_text(self, sample_ids):
        ...
        document_text = np.full((len(sample_ids), self.max_relevant_doc,
                                self.max_document_word), len(self.word2id), dtype=int)
        for i, sample_id in enumerate(sample_ids):
    ...
    def _build_entity_pos(self, sample_ids):
        ...
        for i, sample_id in enumerate(sample_ids):
            ...
            num_nonzero = len(val)
            entity_pos_batch = np.append(
                entity_pos_batch, np.full(num_nonzero, i, dtype=int))
    ...
    def _build_global2local_entity_maps(self):
        ...
        for sample in tqdm(self.data):
            ...
            if self.use_kb:
                self._add_entity_to_map(
                    self.entity2id, sample['subgraph']['entities'], g2l)
            if self.use_doc:
                for relevant_doc in sample['passages']:
                    ...
                    self._add_entity_to_map(
                        self.entity2id, document['document']['entities'], g2l)
                    if 'title' in document:
                        self._add_entity_to_map(
                            self.entity2id, document['title']['entities'], g2l)
