...
class Evaluator_nsm:
    ...
    def evaluate(self, valid_data, test_batch_size=20, write_info=False):
        ...
        for iteration in tqdm(range(num_epoch)):
            batch,IDs,candidate_names,original_data = valid_data.get_batch(iteration, test_batch_size, fact_dropout=0.0, test=True)
            with torch.no_grad():
            ...
            for batch_id in range(batch_size):
                ...
                candidate2prob = []
                dist_log = dict()
                for c, p, s, n in zip(candidates, probs, seed_entities, candidate_names[batch_id]):
                    if c == pad_ent_id:
                        continue
                    dist_log[n] = p
                ...
                if write_info:
                    names = candidate_names[batch_id]
                    tp_obj = obj_list[batch_id]
                    tp_obj['dist'] = dist_log
                    tp_obj['original'] = original_data[batch_id]
                    tp_obj['precison'] = precision
                    tp_obj['recall'] = recall
                    tp_obj['f1'] = f1
                    tp_obj['hit'] = hit
                    tp_obj['cand'] = retrived
                    tp_obj['id'] = IDs[batch_id]
