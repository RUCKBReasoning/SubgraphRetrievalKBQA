...
class SingleDataLoader(BasicDataLoader):
    ...
    def get_batch(self, iteration, batch_size, fact_dropout, q_type=None, test=False, test_with_name=False):
        ...
        if test:
            if not test_with_name:
                ...
                candidate_entities_name = [self.candidate_entities_name[i] for i in sample_ids]
                return self.candidate_entities[sample_ids], \
                    self.query_entities[sample_ids], \
                    (self._build_fact_mat(sample_ids, fact_dropout=fact_dropout)), \
                    q_input, \
                    ...
                    self.answer_dists[sample_ids], \
                    self.answer_lists[sample_ids], \
                    np.array(candidate_entities_name)
