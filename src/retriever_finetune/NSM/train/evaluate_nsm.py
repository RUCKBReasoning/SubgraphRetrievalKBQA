...
class Evaluator_nsm:
    ...
    def evaluate(self, valid_data, test_batch_size=20, write_info=False):
        ...
        if write_info and self.file_write is None:
            filename = os.path.join(self.args['data_folder'], "dist.info")
        ...
        for iteration in tqdm(range(num_epoch)):
            batch = valid_data.get_batch(iteration, test_batch_size, fact_dropout=0.0, test=True, test_with_name=True)
            with torch.no_grad():
                loss, extras, pred_dist, tp_list = self.student(batch[:-2])
                pred = torch.max(pred_dist, dim=1)[1]
            local_entity, query_entities, kb_adj_mat, query_text, \
            seed_dist, true_batch_id, answer_dist, answer_list, local_entity_name = batch
            ...
            for batch_id in range(batch_size):
                ...
                # TMP
                # print("candidates_name: {}".format(candidates_name))
                write_probs = {}
                for c, p, n in zip(candidates, probs, candidates_name):
                    if c == pad_ent_id:
                        continue
                    write_probs[n] = p
                if write_info:
                    self.file_write.write(json.dumps(write_probs) + "\n")
